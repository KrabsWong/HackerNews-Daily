# Spec: GitHub Actions Workflow for Daily Export

## Overview
Automated GitHub Actions workflow that runs daily to export HackerNews articles from the previous day and push them to the TLDR-HackNews24 archive repository.
...
- Depends on existing **daily-export** capability (from previous OpenSpec change)
- Integrates with GitHub Actions infrastructure
- Requires GitHub repository secrets configuration

## Testing Requirements
1. **Workflow Syntax Validation**: YAML syntax must be valid and pass GitHub Actions linter
2. **Manual Trigger Test**: Successful manual execution via `workflow_dispatch`
3. **Secret Access Test**: Verify secrets are accessible and not exposed in logs
4. **Export Generation Test**: Confirm markdown file is generated correctly
5. **Push Verification Test**: Verify file appears in TLDR-HackNews24 repository with correct content
6. **Failure Handling Test**: Simulate failures (missing secret, API error) and verify graceful failure
7. **Scheduled Run Test**: Verify at least one successful scheduled execution at 01:00 UTC

## Security Considerations
- Never log or expose `DEEPSEEK_API_KEY` or `TLDR_REPO_TOKEN` in workflow output
- Use GitHub secrets for all sensitive credentials
- Limit token permissions to minimum required (repo scope only)
- Consider using environment-specific secrets for staging vs production

## Performance Considerations
- Workflow execution time: ~3-5 minutes (depending on number of stories and API response times)
- GitHub Actions free tier limits: 2,000 minutes/month for private repos (unlimited for public repos)
- Daily execution consumes ~3-5 minutes per day = ~90-150 minutes per month
